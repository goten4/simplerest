...
abstract class HttpStatus
{
	...
	{
		return is_numeric($status) && $status >= self::HTTP_BAD_REQUEST;
	}
	
	public static function canHaveBody($status)
	{
		return
			// True if not in 100s
			($status < self::HTTP_CONTINUE || $status >= self::HTTP_OK)
			&& // and not 204 NO CONTENT
			$status != self::HTTP_NO_CONTENT
			&& // and not 304 NOT MODIFIED
			$status != self::HTTP_NOT_MODIFIED;
	}
}
